...
def get_user_information():
  ...
  if b :
    ...
    output ={"user" : json.dumps(b, sort_keys=True, indent=4, default=json_util.default)}
  else:
...
def add_Book_user():
  ...
  user = User.update({'_id':ObjectId(idUser)}, {"$push": {"books": idBook}},upsert = False)
  output = {"Success":200,"user":json.dumps(user, sort_keys=True, indent=4, default=json_util.default)}
  return jsonify({'result' : output})
